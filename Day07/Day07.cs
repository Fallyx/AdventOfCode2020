...
namespace AdventOfCode2020.Day07
{
    class Day07
    {
        ...
        // Parent is key, child is saved inside value with the amount of bags of the child
        private static Dictionary<string, Dictionary<string, int>> bags2 = new Dictionary<string, Dictionary<string, int>>();

        public static void Task1and22()
        {
            using (StreamReader reader = new StreamReader(inputPath))
            {
                string line;
                ...
                while((line = reader.ReadLine()) != null)
                {
                    ...
                    if (!bags2.ContainsKey(splitBags[0]))
                    {
                        bags2.Add(splitBags[0], new Dictionary<string, int>());
                    }
                    ...
                    for(int i = 0; i < splitSubbags.Length; i++) 
                    {
                        ...
                        bags2[splitBags[0]].Add(bagName, num);
                    }
                }

                Console.WriteLine($"Task 1: {ContainsBag2("shiny gold", new HashSet<string>()).Count}");
                Console.WriteLine($"Task 2: {InsideShinyGold2("shiny gold")}");
            }
        }

        private static HashSet<string> ContainsBag2(string bagName, HashSet<string> containBags) 
        {
            foreach(KeyValuePair<string, Dictionary<string, int>> kvPair in bags2)
            {
                if (kvPair.Value.ContainsKey(bagName)) 
                {
                    containBags.Add(kvPair.Key);
                    ContainsBag2(kvPair.Key, containBags);
                }
            }
        ...
        private static int InsideShinyGold2(string bagName)
        {
            int i = 0;

            foreach(KeyValuePair<string, int> kvPair in bags2[bagName])
            {
                i += kvPair.Value;
                i += kvPair.Value * InsideShinyGold2(kvPair.Key);
            }
